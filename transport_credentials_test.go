...
func TestConnCaptureTransportCredentials_Check_TLS(t *testing.T) {
	...
	//cred := conncapture.TransportCredentials(credentials.NewServerTLSFromCert(cert))
	cred := conncapture.TransportCredentials(credentials.NewTLS(&tls.Config{Certificates: []tls.Certificate{*cert}, InsecureSkipVerify: true}))
	dir, err := ioutil.TempDir("", "")
	require.NoError(t, err)
	path := filepath.Join(dir, "socket")
	listener, err := net.Listen("unix", path)
